...
test('Should process extract with relative path', async () => {
  const outputDir = genTMPDirPath();
  const inputConfig = './examples/edge-cases/1/environment.json';
...
test('Should process extract with absolute path', async () => {
  const outputDir = genTMPDirPath();
  const inputConfig = pathJoin(
    process.cwd(),
    './examples/edge-cases/1/environment.json'
  );
...
test('Should not generate docs by default', async () => {
  ...
  await cli(['extract', `-i ${remoteConfig}`, `-o ${outputDir}`, '-f']);
